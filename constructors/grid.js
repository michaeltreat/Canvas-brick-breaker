...
Grid.prototype.drawGraph = function(graphIncrements) {
  ...
  for( let i = 0; i < canvas.width + acc; i += acc){
    ...
    if(i === canvas.width - acc) ctx.strokeText(i + acc, i + 32, 10) // position fix for last value.
  }
  ...
  ctx.beginPath()
  for( let j = 0; j < canvas.height + acc; j += acc){
    ctx.moveTo(0,j)
...
Grid.prototype.createBricks = function(){
  // params = x, y, width, height, id, hp, color , canvas, ctx.

  // for each y axix increment, make a row of bricks.
